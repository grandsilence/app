<template>
  <div>
    <v-input
      :id="name"
      type="text"
      icon-right-color=""
      :value="value || ''"
      :readonly="readonly"
      :placeholder="options.placeholder"
      :icon-left="options.iconLeft"
      :icon-right="options.iconRight"
      :maxlength="length ? +length : null"
      :charactercount="options.showCharacterCount"
      :disabled="!allowEditSecret"
      @input="updateValue"
    ></v-input>
    <br>
    <a :href="orderUrl" target="_blank" rel="noopener noreferrer">Открыть СОДЕРЖИМОЕ ЗАКАЗА</a>
  </div>
</template>

<script>
import mixin from "@directus/extension-toolkit/mixins/interface";
import orderUrlMixin from "./orderUrlMixin";

export default {
  mixins: [mixin, orderUrlMixin],
  computed: {
    /*orderUrl() {
      const baseUrl = window.__DirectusConfig__.frontendUrl;
      return `${baseUrl}/?id=${this.values.id}&secret=${this.value}`;
    },*/
    allowEditSecret() {
      return window.god;
    }
  },
  methods: {
    updateValue(rawValue) {
      let value = rawValue;

      if (this.options.trim) {
        if (!this.value || value.length > this.value.length) {
          value = value.trim();
        }
      }

      this.$emit("input", value);
    }
  }
};
</script>

<style lang="scss" scoped>
.v-input {
  width: 100%;
  max-width: var(--width-medium);
}
</style>
